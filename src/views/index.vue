      ...
      <el-col :xs="24" :sm="24" :md="12" :lg="8">
        <el-card class="update-log">
          <template>
            ...
            <button @click="showCloseConfirm">Close WebSocket Connection</button>
          </div>
          </template>
        </el-card>
      </el-col>
...
export default {
  ...
  methods: {
    ...
    connectWebSocket() {
      ...
      this.message += `客户端 id = ${userId}<br/>`;

      // 判断当前浏览器是否支持WebSocket
      if ("WebSocket" in window) {
        // 改成你的地址
        this.socket = new WebSocket(`ws://localhost:8080/imserver/app/${userId}`);
      } else {
